	...
	It("should deploy coil by Argo CD", func() {
		...
		Eventually(func() error {
			...
			stdout, stderr, err := execAt(boot0, "kubectl", "patch",
				"-n", argoCDNamespace, "app", "coil", "--type=merge",
				"-p", `'{"spec":{"source":{"targetRevision":"`+commitID+`"}}}'`)
			if err != nil {
				return fmt.Errorf("stdout: %s, stderr: %s, err: %v", stdout, stderr, err)
			}
			stdout, stderr, err = execAt(boot0, "argocd", "app", "sync", "coil", "--timeout", "20")
			if err != nil {
				return fmt.Errorf("stdout: %s, stderr: %s, err: %v", stdout, stderr, err)
			}
			return nil
		...
		Eventually(func() error {
			stdout, stderr, err := execAt(boot0, "kubectl", "get", "app", "coil", "-n", argoCDNamespace, "-o", "json")
			if err != nil {
				return fmt.Errorf("stdout: %s, stderr: %s, err: %v", stdout, stderr, err)
			}
