...
function setup_master() {
    TARGET_POD=$1
    mysqlsh --no-wizard --sql --uri ${MYSQL_OPERATOR_USER}:${MYSQL_OPERATOR_PASSWORD}@${TARGET_POD}.${MYSQL_CLUSTER_DOMAIN}-copy:3306 << EOS
...
function change_master_to() {
    ...
    mysqlsh --no-wizard --sql --uri ${MYSQL_OPERATOR_USER}:${MYSQL_OPERATOR_PASSWORD}@${TARGET_POD}.${MYSQL_CLUSTER_DOMAIN}-copy:3306 << EOS
CHANGE MASTER TO
  MASTER_HOST = '${master}.${MYSQL_CLUSTER_DOMAIN}-copy',
  MASTER_PORT = 3306,
